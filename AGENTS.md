...
**Default home directory**: `~/.classgo/` — created on first run with sensible defaults (current user as superadmin). Contains `data/`, `classgo.db`, and `config.json`. Override with `--data-dir` or `config.json`.
