...
func TodayAttendees(db *sql.DB) ([]models.Attendance, error) {
	rows, err := db.Query(
		"SELECT id, student_id, student_name, device_type, check_in_time, check_out_time FROM attendance WHERE date(check_in_time) = date('now','localtime') ORDER BY check_in_time DESC",
	)
	if err != nil {
		return nil, err
	}
...
func AttendeesByDateRange(db *sql.DB, from, to, studentID, teacherID, parentID string) ([]models.Attendance, error) {
	q := "SELECT DISTINCT a.id, a.student_id, a.student_name, a.device_type, a.check_in_time, a.check_out_time FROM attendance a"
	var joins []string
	var wheres []string
	var args []any
...
func scanAttendees(rows *sql.Rows) ([]models.Attendance, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.StudentID, &a.StudentName, &a.DeviceType, &checkIn, &checkOut); err != nil {
			return nil, err
		}
